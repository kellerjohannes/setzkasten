          ...
          (if (listp candidate)
              (case (first candidate)
                ...
                (:key-signature
                 ...
                 (cancel-accidental parser-state)))
              ;; here the glyph processing happens (non-list candidate)
              (let ((candidate-key (make-keyword candidate)))
                (when (member candidate-key *f-clef-triggers*)
                  (cancel-f-clef-flag parser-state))
                (if (member candidate-key *meter-signatures* :key #'first)
                    (setf (meter parser-state) (cdr (assoc candidate-key *meter-signatures*)))
...
(defmethod parse-glyph ((score score) (parser-state parser-state) keyword-glyph dottedp)
  (case keyword-glyph
    ...
    (:sinsh3 (set-accidental parser-state :single-sharp 3))
    (:sh4 (set-accidental parser-state :sharp 4))
    (:sh5 (set-accidental parser-state :sharp 5))
    (:sinsh5 (set-accidental parser-state :single-sharp 5))
    (:sh6 (set-accidental parser-state :sharp 6))
    (:sinsh6 (set-accidental parser-state :single-sharp 6))
    (:sh7 (set-accidental parser-state :sharp 7))
    (:sinsh7 (set-accidental parser-state :single-sharp 7))
    (:sh7blue (set-accidental parser-state :sharp 7))
    ...
    (:fl8blue (set-accidental parser-state :flat 8))
    ...
    (:longa6 (add-note score parser-state :longa dottedp 6 nil))
    ...
    (:sb7blue (add-note score parser-state :semibrevis dottedp 7 nil))
    (:sb8 (add-note score parser-state :semibrevis dottedp 8 nil))
    (:sb8blue (add-note score parser-state :semibrevis dottedp 8 nil))
    ...
    (:sbd7blue (add-note score parser-state :semibrevis dottedp 7 :dot))
    (:sbd8 (add-note score parser-state :semibrevis dottedp 8 :dot))
    (:sbd8blue (add-note score parser-state :semibrevis dottedp 8 :dot))
