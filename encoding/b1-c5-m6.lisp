`((:header
   (:filename "b1-c5-m6")
   (:alt-name "")
   (:comment "")
   (:creator "Vicentino21 / Johannes Keller")
   (:title "Mano enarmonica discendente con i semitoni minori, nel principio del tono per le sette lettere della mano, con il tono diviso in quattro parti: e come a da leggere")
   (:section-headings
    (s1 "da A a G")
    (s2 "da G a F")
    (s3 "da F a E")
    (s4 "da E a D")
    (s5 "da D a C")
    (s6 "da C a B")
    (s7 "da B a A"))
   (:section-captions
    (s1 "per natura: la-sol-fa-la-sol\\mi e re ascendino")
    (s2 "per natura: sol-fa-la-sol-fa\\re e ut ascendeno")
    (s3 "per natura: fa-mi-la per discendere\\fa-mi-mi per ascendere")
    (s4 "per ♮: la-sol-fa-mi-re\\la-sol-fa-la-sol\\mi ascende")
    (s5 "per ♭: la-sol-fa-la-sol\\per ♮: sol-fa-la-sol-fa")
    (s6 "per ♮: fa-mi-mi")
    (s7 "per ♮: mi-mi-fa-mi-re\\per ♭: fa-mi-la per discendere"))
   (:voice-labels
    (:all v1 "")))
  (:preamble-type-imitation
   (:width nil)
   (:height nil)
   (:background ,*score-bg-color*))
  (:data
   (:text 70 58 (0 2210 "ᴍano ᴇnarmonica diſcendente con i ſemitoni minori, nel principio del tono per le ſette lettere della"))
   (:text 70 58 (0 1484 "ᴍano, con il tono diuiſo in quattro parti:& come ſi à da leggere."))
   (:text 110 58
          (162 nil "da ᴀ.")
          (460 nil "à ɢ.")
          (734 nil "da ɢ.")
          (1036 nil "à ꜰ.")
          (1320 nil "da ꜰ.")
          (1524 nil "à ᴇ.")
          (1738 nil "da ᴇ.")
          (1960 nil "à ᴅ."))
   (:music 2210
           (:voice v1)
           (:section s1
                     )
           (:f-clef) max7 fclef7 b38 sb9 b38 fl9 b38 sbd9 b38 fl9 b38 sb9 b38 sbd8 b38 sb8 bl
           (:section s2)
           sb8 b38 fl8 b38 sbd8 b38 fl8 b38 sb8 b38 sbd7 b38 sb7 bl
           (:section s3)
           b38 b22 sb7 b38 sbd6 b38 sb6 b38 b22 bl
           (:section s4)
           sb6 b22 fl6 sbd6 b22 fl6 sb6 sbd5 b22 sb5 bl)
   (:text 70 58
          (10 nil "ꝑ n.")
          (130 390 "la. ſol. fa. la. ſol.")
          (724 530 "per n. ſol. fa. la. ſol. fa.")
          (1302 412 "ꝑn.fa.mi.la.ꝑ diſ.")
          (1742 454 "ꝑ♮. la.ſol.fa.mi.re."))
   (:text 70 58
          (150 490 "mi.& re. aſcendino")
          (864 444 "re.& ut.aſcendeno")
          (1374 258 "fa.mi.mi.")
          (1842 350 "la.ſol.fa.la. ſol."))
   (:text 70 58
          (1382 218 "ꝑ aſcend.")
          (1870 262 "mi aſcende,"))
   (:text 100 58
          (176 nil "da ᴅ.")
          (500 nil "à ᴄ.")
          (782 nil "da ᴄ.")
          (986 nil "à ʙ.")
          (1198 nil "da ʙ.")
          (1502 nil "à ᴀ."))
   (:music 2210
           (:section s5)
           (:f-clef) max7 fclef7 b38 sb5 b38 fl5 b38 sbd5 b38 fl5 b38 sb5 b38 sbd4 b38 sb4 b22 bl
           (:section s6)
           b38 sb4 b38 sbd3 b38 sb3 b38 bl
           (:section s7)
           b22 sb3 b38 fl3 b38 sbd3 b38 fl3 b38 sb3 b38 sbd2 b38 sb2 b38 bl
           b38 b38 b38 b38 b38 b38 b38 b38 b38 b38 b38 b38)
   (:text 70 58
          (10 nil "ꝑ ♭.")
          (156 468 "la. ſol. fa. la. ſol.")
          (744 358 "ꝑ ♮.fa.mi.mi.")
          (1142 580 "ꝑ ♮.mi. mi. fa. mi. re."))
   (:text 70 58
          (10 nil "ꝑ ♮.")
          (144 480 "ſol. fa. la. ſol. fa.")
          (1124 nil "ꝑ ♭.")
          (1452 446 "fa.mi.la.ꝑ diſcend."))))
