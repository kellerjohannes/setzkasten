`((:header
   (:filename "b3-c28-m1")
   (:alt-name "m3.059")
   (:comment "q021_s106, origin: 877")
   (:creator "Vicentino21 / Johannes Keller")
   (:title (:alt (:it "Esempio delle cadentie naturali et accidentali,\\a due voci, della musica participata e mista,\\lequali fuggano la sua conclusione")
                 (:de "Beispiel der naturali und accidentali Kadenzen\\mit zwei Stimmen der musica participata und mista,\\welche ihren Abschluss fliehen.")
                 (:en "Example of naturali and accidentali cadences,\\in two voci, of musica participata and mista\\which evade their conclusion")))
   (:section-headings
    ;; JK: »A due voci« steht im Original nur ein einziges Mal und wird für sämtliche folgende
    ;; Sections ergänzt. Gut so?
    ;; kir: hm, ist vielleicht nicht nötig (weil redundant)
    ;; OPEN
    (s1 (:alt (:it "A due voci")
              (:de "für zwei Stimmen")
              (:en "In two voci")))
    (s2 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))
    (s3 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))
    (s4 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))
    (s5 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))
    (s6 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))
    (s7 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))
    (s8 (:alt (:it "[A due voci]")
              (:de "[für zwei Stimmen]")
              (:en "In two voci")))))
  (:data
   (:text 70 58
          (70 2054 "Eſſempio delle ᴄadentie naturali& accidentali,à due uoci,della muſica participata &"))
   (:text 130 58
          (636 943 "miſta,lequali fuggano la ſua concluſione."))
   (:music 2230
           (:voice v1)
           (:section s1) cclef7 b22 mrest7 b22 m7 b22 dot8 b22 cr6 b38 cr5 b38 m6 b38 sb9 bl
           (:section s2) m5 b38 m7 dot8 cr6 b22 cr5 b38 m6 b38 sb2 bl
           (:section s3) m1 b38 m4 dot4 cr3 b22 cr2 b38 sh3 b38 m3 b38 sb6 bl
           (:section s4) b22 mrest3 b22 sb4 b38 m3 b38 fl6 b22 sb6 b22 bl)
   (:vspace 40)
   (:music 2213
           (:voice v2)
           (:section s1) (:f-clef) max7 fclef7 b38 b38 sb7 b38 b38 sb5 b38 b38 sb4 b38 b38 b22 b22 bl
           (:section s2) b38 b38 sb7 b38 b38 sb5 b38 b38 sb4 b38 b38 bl
           (:section s3) b38 b38 b38 sb3 b38 b38 b38 sb2 b38 b38 b38 sb1 b38 b38 b38 bl
           (:section s4) b38 sb3 b38 b38 sb2 b38 b38 sb1 b38 bl)
   (:vspace 20)
   (:text 110 58
          (949 nil "A due uoci."))
   (:music 2225
           (:voice v1)
           (:section s5) (:newline) cclef7 b22 mrest5 b22 sb5 b38 sh4 b38 m4 b38 sh7 b38 sb7 b38 bl
           (:section s6) b22 mrest5 b38 sb5 b38 sh4 b38 m4 b38 sb7 b38 bl
           (:section s7) b38 mrest5 b38 sb5 b38 sh4 b38 m4 b38 sb8 b38 bl
           (:section s8) b38 mrest5 b38 sb5 b38 sh4 b38 m4 b38 sh8 b38 sb8 b22 b22 bl b38)
   (:music 2213
           (:voice v2)
           (:section s5) (:f-clef) max7 fclef7 b38 b38 sb4 b38 b38 sb3 b38 b38 sb2 b38 b22 b22 bl
           (:section s6) b38 b38 sb4 b38 b38 sb3 b38 b38 sb2 b38 b38 bl
           (:section s7) b38 b38 sb7 b38 b38 sb6 b38 b38 sb5 b22 b22 bl
           (:section s8) b38 b38 b22 b22 sb2 b22 b22 b22 sb6 b38 b38 b22 sb3 b38 b38 b38 bl b38)))
