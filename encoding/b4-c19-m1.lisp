`((:header
   ...
   (:title (:alt (:it "Essempio à due uoci, delle quinte antecedenti,\\& sussequenti, con il buono & megliore procedere\\di ♮ & di ♭ & del Diesis cromatico.")
                 (:de "Beispiel für zwei Stimmen von voranstehenden\\und nachfolgenden quinte, mit dem guten und besseren Fortschreiten\\mit ♮ und ♭ und der diesis cromatico")
                 (:en "Example in two voci of the quinte before and after, with good and better progressions\\of ♮ and of ♭ and of the diesis cromatico")))
   (:section-headings
    (s1 (:alt (:it " \\Quinta antecedente")
              (:de " \\voranstehende quinta")
              (:en " \\Quinte before")))
    (s2 (:alt (:it "Quinta antecedente\\& sussequente")
              (:de "voranstehende\\und nachfolgende quinta")
              (:en "Quinte before\\and after")))
    (s3 (:alt (:it " \\buono procedere")
              (:de " \\gutes Fortschreiten")
              (:en " \\good progression")))
    (s4 (:alt (:it " \\megliore")
              (:de " \\besser")
              (:en " \\better")))
    ...
    (s7 (:alt (:it "modo di procedere\\con una parte")
              (:de "Art und Weise, mit einer Stimme\\fortzuschreiten")
              (:en "method of proceeding\\with one part")))))
  ...
  (:data
   ...
   (:music 2230
           ...
           (:alt (:diplomatic sb7)
                 ...
                 (:comment "In allen Exemplaren: C. Alternativ könnte dieses Beispiel als zwei separate einstimmige Beispiele gelesen werden, wie es die Überschrift suggeriert. In diesem Fall ist keine Korrektur notwendig.")
                 (:flag :ready) (:id 1))
