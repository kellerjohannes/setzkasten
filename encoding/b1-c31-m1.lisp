`((:header
   (:filename "b1-c31-m1")
   (:alt-name "m1.039")
   (:comment "")
   (:creator "Vicentino21 / Johannes Keller")
   (:title "Essempio dei gradi ti terza più di maggiore incomposto e composto")
   (:section-headings
    (s1 ""))
   (:section-captions
    (s1 "grado di terza\\più di maggiore\\incomposto")
    (s2 "grado di terza\\più di maggiore\\composto")
    (s3 "grado di terza un\\comma più di maggiore\\incomposto e composto")
    (s4 "grado di terza\\più di maggiore\\composto")
    (s5 "grado di terza\\più di maggiore\\incomposto")
    (s6 "grado di terza\\più di maggiore\\composto")
    (s7 "grado di terza\\più di maggiore\\incomposto"))
   (:voice-labels
    (:all v1 "")))

  (:data
   (:text 150 58
          (284 1607 "ᴇſſempio de i gradi di ᴛerza più di maggiore incompoﬆo & compoſto."))
   (:music 2209
           (:voice v1)
           (:section s1) cclef7 b22 b22 sb0 b38 sbd2 b38 bl
           (:section s2) sb3 b22 sb4 b22 sbd5 bl
           (:section s3) fl6 sb6 b38 sbc8 b22 fl5 sb5 sb6 b22 sbc7 bl
           (:section s4) fl6 b22 sb6 b22 sb7 b22 sbc8 bl
           (:section s5) (:newline) fl8 b22 sb8 b22 sb6 bl
           (:section s6) fl6 b22 sb6 b22 sb5 b22 sb4 b22 sh3 b22 sb3 bl
           (:section s7) sbd4 b27 fl2 b22 sb2 bl)
   (:text 70 58
          (39 nil "grad.di 3.")
          (307 nil "gra.di 3.")
          (548 nil "grad. di terza un")
          (989 nil "grad. di 3.")
          (1275 nil "gra.di 3.")
          (1496 nil "grad.di terza più")
          (1935 nil "gra. di 3."))
   (:text 70 58
          (45 nil "più di ma.")
          (311 nil "più di ma.")
          (564 nil "cōma più di mag.")
          (982 nil "più di ma.")
          (1268 nil "più di ma.")
          (1502 nil "di magg.comp.")
          (1932 nil "più di ma."))
   (:text 70 58
          (41 nil "incomp. .")
          (302 nil "compoſto")
          (570 nil "incomp. et comp.")
          (977 nil "compoſto")
          (1256 nil "incomp.")
          (1930 nil "incomp."))))
