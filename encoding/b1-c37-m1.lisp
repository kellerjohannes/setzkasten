`((:header
   (:filename "b1-c37-m1")
   (:alt-name "m1.045")
   (:comment "q021_s047, origin: 369")
   (:creator "Vicentino21 / Johannes Keller")
   (:title (:alt (:it "Essempio del salto de quinta imperfetta naturale e accidentale, incomposto")
                 (:de "")
                 (:en "Example of the leap of a quinta imperfetta naturale and accidentale, incomposto")))
   (:section-captions
    (s1 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))
    (s2 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))
    (s3 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))
    (s4 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))
    (s5 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))
    (s6 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))
    (s7 (:alt (:it "salto di quinta\\imperfetta\\naturale\\incomposto")
              (:de "")
              (:en "leap of a quinta\\imperfetta\\naturale\\incomposto")))))
  (:data
   (:text 80 58
          (200 1805 "ᴇſſempio del ſalto de Ɋuinta imperfetta,naturale & accidentale,incompoﬆo."))
   (:music 2219
           (:voice v1)
           (:section s1) (:f-clef) max7 fclef7 b38 sb3 b38 sb7 b38
           (:section s2) cclef5 b38 sb4 b38 b38 sb8 b38 bl
           (:section s3) b38 sb0 b38 fl4 b38 sb4 b38
           (:section s4) cclef5 b38 sb3 fl7 b38 sb7 b38
           (:section s5) cclef5 b38 fl4 b38 sb4 b38 sbd7 bl
           (:section s6) sb2 b38 fl6 b38 sb6 b22 bl
           (:section s7) sb5 b38 sh1 b38 sb1 b38 bl)
   (:text 70 58
          (48 nil "ſalto di quinta")
          (419 nil "ſalto di 5.")
          (684 nil "ſalto di quin.")
          (1016 nil "ſalto di 5.")
          (1305 nil "ſalto di quin.")
          (1641 nil "ſalto di 5.")
          (1891 nil "ſalto di quinta"))
   (:text 70 58
          (52 nil "imper.nat.&")
          (425 nil "imp.nat.")
          (698 nil "imp.accid.&")
          (1023 nil "imp. accid.")
          (1320 nil "imp.accid.&")
          (1648 nil "imp.acci.")
          (1889 nil "imper.acci. et"))
   (:text 70 58
          (52 nil "incompoſto")
          (436 nil "incomp.")
          (700 nil "incompoſto")
          (1028 nil "incompoſto")
          (1332 nil "incompoſto")
          (1644 nil "et incom.")
          (1878 nil "incompoﬆo"))))
