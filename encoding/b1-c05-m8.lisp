`((:header
   (:filename "b1-c05-m8")
   (:alt-name "m1.010")
   (:comment "q021_s025, origin: 502")
   (:creator "Vicentino21 / Johannes Keller")
   (:title (:alt (:it "Mano enarmonica, con semitoni maggiori discendenti nel principio del tono,\\con il tono, in quattro parti diviso, per le sette lettere della mano.")
                 (:de "")
                 (:en "Enharmonic hand, with descending _semitoni maggiori_ at the beginning of the tono,\\with the _tono_ divided into four parts by the seven letters of the hand")))
   (:section-headings
    (s1 (:alt (:it "da A a G")
              (:de "")
              (:en "from A to G")))
    (s2 (:alt (:it "[da A a G]")
              (:de "")
              (:en "from G to F")))
    (s3 (:alt (:it "da F a E")
              (:de "")
              (:en "from F to E")))
    (s4 (:alt (:it "da E a D")
              (:de "")
              (:en "from E to D")))
    (s5 (:alt (:it "da D a C")
              (:de "")
              (:en "from D to C")))
    (s6 (:alt (:it "da C a B")
              (:de "")
              (:en "from C to B")))
    (s7 (:alt (:it "da B a A")
              (:de "")
              (:en "from B to A"))))
   (:section-captions
    (s1 (:alt (:it "per natura: la-sol-fa-la-sol\\mi e re per ascendere")
              (:de "")
              (:en "")))
    (s2 (:alt (:it "[per natura: sol-fa-la-sol-fa\\re et ut per ascendere]")
              (:de "")
              (:en "")))
    (s3 (:alt (:it "per natura: fa-mi-la\\fa-mi-mi")
              (:de "")
              (:en "")))
    (s4 (:alt (:it "per ♮: la-sol-fa-mi-re\\la-sol-fa-la-sol\\mi per ascendere")
              (:de "")
              (:en "")))
    (s5 (:alt (:it "per ♭: la-sol-fa-la-sol\\per ♮: sol-fa-la-sol-fa\\re per ascendere")
              (:de "")
              (:en "")))
    (s6 (:alt (:it "per natura: fa-mi-mi")
              (:de "")
              (:en "")))
    (s7 (:alt (:it "per ♭: fa-mi-la\\fa-mi-mi")
              (:de "")
              (:en "")))))
  (:data
   (:text 70 58 (0 2198 "ᴍano ᴇnarmonica , con ſemitoni maggiori diſcendenti nel principio del tono,con il tono, in quattro"))
   (:text 70 58 (0 976 "parti diuiſo , per leſette lettere della ᴍano."))
   (:text 110 58
     (130 nil "da ᴀ.")
     (364 nil "à ɢ.")
     (525 nil
          (:alt (:diplomatic "")
                (:idealised "da ɢ.")
                (:comment "Dieser Text wurde ergänzt.") (:flag :draft) (:id 0)))
     (750 nil
          (:alt (:diplomatic "")
                (:idealised "à ꜰ.")
                (:comment "Dieser Text wurde ergänzt.") (:flag :draft) (:id 0)))
     ((:alt (:diplomatic 516)
            (:idealised 910)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      nil "da ꜰ.")
     ((:alt (:diplomatic 660)
            (:idealised 1030)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      nil "à ᴇ.")
     ((:alt (:diplomatic 794)
            (:idealised 1150)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      nil "da ᴇ.")
     ((:alt (:diplomatic 1010)
            (:idealised 1400)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      nil "à ᴅ.")
     ((:alt (:diplomatic 1234)
            (:idealised 1530)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      nil "da ᴅ.")
     ((:alt (:diplomatic 1478)
            (:idealised 1800)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      nil "à ᴄ.")
     ((:alt (:diplomatic 1630)
            (:idealised 1920)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      234 "da ᴄ. à ʙ.")
     ((:alt (:diplomatic 1916)
            (:idealised 2200)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      238 "da ʙ. à ᴀ."))
   (:music
           (:alt (:diplomatic 2198)
                 (:idealised 2450)
                 (:comment "Die Breite der Notenzeile wurde angepasst, um die Ergänzung von Noten 6-10 zu ermöglichen.") (:flag :draft) (:id 0))

           (:voice v1)
           (:section s1) (:f-clef) max7 fclef7 b22 sb9 fl9 sbd9 sh8 sb8 b22 sbd8 b22 sb8 bl

           (:alt (:diplomatic)
                 (:idealised (:section s2) sb8 fl8 sbd8 sh7 sb7 sbd7 sb7 bl)
                 (:comment "Noten 6-10 (Schritt von G zu F): Im Original existiert diese Notenfolge nicht, ist aber inhaltlich plausibel.") (:flag :draft) (:id 0))

           (:section s3) sb7 b22 sbd6 b22 sb6 bl
           (:section s4) sb6 fl6 sbd6 sh5 sb5 b22 sbd5 b22 sb5 bl
           (:section s5) (:newline) sb5 fl5 sbd5 sh4 sb4 b22 sbd4 b22 sb4 bl
           (:section s6) sb4 b22 sbd3 b22 sb3 bl
           (:section s7) fl3 sb3 b22 sbd2 b22 sb2 b22 bl)
   (:text 70 58
     (10 440 "ꝑ n. la.ſol.fa.la.ſol.")
     (454 420
          (:alt (:diplomatic "")
                (:idealised "ꝑ n. ſol.fa.la.ſol.fa.")
                (:comment "Dieser Text wurde ergänzt.") (:flag :draft) (:id 0)))
     ((:alt (:diplomatic 494)
            (:idealised 890)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      274 "ꝑ n.fa.mi.la.")
     ((:alt (:diplomatic 782)
            (:idealised 1160)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      400 "ꝑ ♮.la.ſol.fa.mi.re.")
     ((:alt (:diplomatic 1220)
            (:idealised 1560)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      354 "ꝑ ♭.laſolfalaſol.")
     ((:alt (:diplomatic 1600)
            (:idealised 1920)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      268 "ꝑ n.fami.mi.")
     ((:alt (:diplomatic 1886)
            (:idealised 2200)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      290 "ꝑ ♭.fa.mi.la."))
   (:text 70 58
     (90 370 "mi.et re.ꝑ aſcen.")
     (474 nil "re. et ut. ꝑ aſcen.")
     ((:alt (:diplomatic 524)
            (:idealised 890)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      218 "fa.mi.mi.")
     ((:alt (:diplomatic 824)
            (:idealised 1160)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      340 "la.ſol.fa.la.ſol.")
     ((:alt (:diplomatic 1172)
            (:idealised 1560)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      434 "ꝑ ♮.ſol.fa.la.ſol.fa.")
     ((:alt (:diplomatic 1972)
            (:idealised 2200)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      218 "fa.mi.mi."))
   (:text 70 58
     ((:alt (:diplomatic 838)
            (:idealised 1160)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      336 "mi. per aſcend.")
     ((:alt (:diplomatic 1264)
            (:idealised 1560)
            (:comment "Dieser Text wurde nach rechts verschoben.") (:flag :draft) (:id 0))
      294 "re. ꝑ aſcend."))))
