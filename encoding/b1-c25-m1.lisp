`((:header
   (:filename "b1-c25-m1")
   (:alt-name "")
   (:comment "")
   (:creator "Vicentino21 / Johannes Keller")
   (:title "Essempio del grado o salto della terza manco di minore,\\overo minima, composto e incomposto")
   (:section-headings
    (s1 ""))
   (:section-captions
    (s1 "grado di terza\\minima\\incomposto")
    (s2 "grado di terza\\minima\\composta")
    (s3 "grado di terza\\minima\\incomposta")
    (s4 "grado di terza\\minima\\composta")
    (s5 "grado di terza\\minima\\incomposta")
    (s6 "grado di terza\\minima\\composta")
    (s7 "grado di terza\\minima\\incomposta"))
   (:voice-labels
    (:all v1 "")))
  (:preamble-type-imitation
   (:width nil)
   (:height nil)
   (:background ,*score-bg-color*))
  (:data
   (:text 90 58
          (0 2201 "ᴇſſempio del grado ò ſalto della ᴛerza manco di minore, ouero minima compoſto & incompoſto."))
   (:music 2208
           (:voice v1)
           (:section s1)
           cclef7 b22 sb1 b22 sh2 b22 sb2 b22 bl
           (:section s2)
           sb3 b22 sb4 b22 sh4 b22 sb4 bl
           (:section s3)
           b22 sb4 b22 sh5 b22 sb5 b38 bl
           (:section s4)
           sh5 b22 sb5 b22 sb5 b22 sb4 bl
           (:section s5)
           b22 sh4 b22 sb4 b22 sb3 b22 bl
           (:section s6)
           sh3 b22 sb3 b22 sb3 b22 fl2 b22 sb2 bl
           (:section s7)
           b22 sh2 b38 sb2 b38 sb1 bl)
   (:text 70 58
          (44 nil "grado di 3.")
          (335 nil "grado di 3.")
          (641 nil "grado di 3.")
          (928 nil "grado di 3.")
          (1237 nil "grado di 3.")
          (1541 nil "grado di terza")
          (1903 nil "grado di 3."))
   (:text 70 58
          (50 nil "mini.incom.")
          (336 nil "mini.comp.")
          (639 nil "mini.incom.")
          (934 nil "mini.comp.")
          (1245 nil "mini.incom.")
          (1541 nil "mini.comp.")
          (1911 nil "mini.incom."))))
