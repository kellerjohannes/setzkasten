`((:header
   (:filename "b1-c5-m3")
   (:alt-name "m1.005")
   (:comment "")
   (:creator "Vicentino21 / Johannes Keller")
   (:title "Dimostratione della mano cromatica con li semitoni maggiori ascendenti")
   (:section-headings
    (s1 "Alamire")
    (s2 "Bfa♭mi")
    (s3 "B et C")
    (s4 "Csolfaut")
    (s5 "Dlasolre")
    (s6 "Elami")
    (s7 "Ffaut"))
   (:section-captions
    (s1 "per natura,\\per ♭,\\per ♮:\\la, mi, re,\\ut, fa, sol")
    (s2 "per ♭,\\per ♮:\\fa, mi,\\lo, sol, re, ut")
    (s3 "per ♮,\\per natura:\\mi, fa, sol,\\la, re, ut")
    (s4 "per natura,\\per ♭:\\sol, fa, ut,\\re, mi, la")
    (s5 "per natura,\\per ♭:\\la, sol, re,\\fa, mi, ut")
    (s6 "per natura,\\per ♮:\\la, mi, fa,\\ut, re, sol")
    (s7 "per natura,\\per ♭:\\fa, ut, re,\\mi, sol, la"))
   (:voice-labels
    (:all v1 "")))
  (:preamble-type-imitation
   (:width nil)
   (:height nil)
   (:background ,*score-bg-color*))
  (:data
   (:text 70 58 (231 1648 "ᴅimoſtratione della ᴍano ᴄromatica con li ſemitoni maggiori aſcendenti."))
   (:text 100 58
          (42 nil "ᴀ la mire")
          (287 nil "ʙ fa ♭ mi")
          (622 nil "ʙ et ᴄ")
          (866 nil "ᴄ ſol fa ut")
          (1202 nil "ᴅ la ſol re")
          (1527 nil "ᴇ la mi")
          (1778 nil "ꜰ fa ut"))
   (:music 2200
           (:voice v1)
           (:section s1)
           b38 (:f-clef) max7 fclef7 b38 sb2 b38 bl
           (:section s2)
           fl3 b38 sb3 b38 nat3 b38 sbd3 bl
           (:section s3)
           b38 sb3 b38 sb4 b38 bl
           (:section s4)
           b38 sb4 b38 fl5 b38 sb5 b38 bl
           (:section s5)
           b38 sb5 b38 fl6 b38 sb6 b38 bl
           (:section s6)
           b38 sb6 b38 sb7 b38 bl
           (:section s7)
           b38 sb7 b38 fl8 b38 sb8 b38 bl b70)
   (:text 70 58
          (43 nil "la") (126 nil "mi re")
          (288 nil "fa") (391 nil "mi")
          (619 nil "mi fa ſol")
          (872 nil "ſol fa ut")
          (1204 nil "la ſol re")
          (1541 nil "la mi fa")
          (1794 nil "fa ut re"))
   (:text 70 58
          (43 nil "ut fa ſol")
          (295 nil "la ſol re ut")
          (635 nil "la reut")
          (883 nil "re mi la")
          (1195 nil "fa mi ut")
          (1542 nil "ut re ſol")
          (1799 nil "mi ſol la"))
   (:text 70 58
          (5 309 "ꝑ n. ꝑ ♭.ꝑ ♮.")
          (325 218 "ꝑ ♭. ꝑ ♮.")
          (644 480 "per ♮.ꝑ n. ꝑ n. ꝑ ♭.")
          (1198 220 "ꝑ n. ꝑ ♭.")
          (1542 227 "ꝑ n. ꝑ ♮.")
          (1807 213 "ꝑ n.ꝑ ♭."))))
