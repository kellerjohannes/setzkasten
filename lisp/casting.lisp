...
(defmethod cast ((stencil glyph-notehead-flag))
  ...
  (with-accessors ((thickness-bold flag-thickness-bold)
	...
	(let* ((stem-length (* (stem-length (stem-component stencil)) unit-length))
	       ...
	       (p (vec:create (h-center stencil)
			      (- notehead-y (* 0.5 (calculate-notehead-height stencil)))))
	       (q (vec:create (+ (h-center stencil) (* 0.5 stem-width-head))
			      (vec:y-coord p)))
	       (a (vec:add m (vec:create (* 0.5 stem-width-factor stem-width-head) 0)))
	       (b (vec:create (+ (h-center stencil) y-offset)
			      (+ (vec:y-coord m) (* corner-level stem-length))))
	       (h (vec:add a (vec:create (- (vec:hypothenuse-a-o (vec:sin-between-vectors (vec:subtract b a)
											  (vec:subtract a m))
								 thickness-bold))
					  0))))
	  ;; (when (eq (stem-direction stencil) :down)
	  ;;   (transform-coordinates #'vec:mirror-y notehead-y m))
	  ;; TODO when semi-flag ...
	  (push (output-debug-circle m) (svg-data stencil))
	  (push (output-debug-circle n) (svg-data stencil))
	  (push (output-debug-circle p) (svg-data stencil))
	  (push (output-debug-circle q) (svg-data stencil))
	  (push (output-debug-circle a) (svg-data stencil))
	  (push (output-debug-circle h) (svg-data stencil))
	  ;; (push (output-path "even-odd" "blue" ; (ink-color stencil)
	  ;; 		     `((m ,a) (l ,b) (l ,c) (l ,d) (l ,f) (l ,g) (l ,h) (c)))
	  ;; 	(svg-data stencil))
	  ))))
